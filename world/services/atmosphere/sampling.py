...
def _weather_from_grid_at_time(
    ...
    *,
    ...
    wind_speed = point.wind_speed_m_s
    direction = point.wind_direction_degrees
    cloud_cover = values["cloud_cover"]
    physical_rate = max(0.0, values["precipitation_rate"])
    precipitation_rate_mm_h = physical_rate * 3600.0
    rain_fraction, snow_fraction = rain_and_snow_fraction(temperature, settings)
    q_c = values["cloud_condensate_specific_humidity"]
    ...
    return WeatherState(
        ...
        pressure_hpa=round(values["pressure_hpa"], 1),
